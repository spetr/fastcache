...
func TestARCGet(t *testing.T) {
	size := 1000
	gc := buildTestCache(t, cacheTypeARC, size)
...
func TestLoadingARCGet(t *testing.T) {
	size := 1000
	numbers := 1000
	testGetCache(t, buildTestLoadingCache(t, cacheTypeARC, size, loader), numbers)
}

func TestARCLength(t *testing.T) {
	gc := buildTestLoadingCacheWithExpiration(t, cacheTypeARC, 2, time.Millisecond)
	gc.Get("test1")
	gc.Get("test2")
	gc.Get("test3")
	length := gc.Len(true)
...
func TestARCEvictItem(t *testing.T) {
	cacheSize := 10
	numbers := cacheSize + 1
	gc := buildTestLoadingCache(t, cacheTypeARC, cacheSize, loader)
...
func TestARCGetIFPresent(t *testing.T) {
	testGetIFPresent(t, cacheTypeARC)
}

func TestARCHas(t *testing.T) {
	gc := buildTestLoadingCacheWithExpiration(t, cacheTypeARC, 2, 10*time.Millisecond)
