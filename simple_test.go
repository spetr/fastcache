...
func TestSimpleGet(t *testing.T) {
	size := 1000
	gc := buildTestCache(t, cacheTypeSimple, size)
...
func TestLoadingSimpleGet(t *testing.T) {
	size := 1000
	numbers := 1000
	testGetCache(t, buildTestLoadingCache(t, cacheTypeSimple, size, loader), numbers)
}

func TestSimpleLength(t *testing.T) {
	gc := buildTestLoadingCache(t, cacheTypeSimple, 1000, loader)
	gc.Get(ctx, "test1")
	gc.Get(ctx, "test2")
	length := gc.Len(true)
	expectedLength := 2
...
func TestSimpleEvictItem(t *testing.T) {
	cacheSize := 10
	numbers := 11
	gc := buildTestLoadingCache(t, cacheTypeSimple, cacheSize, loader)
...
func TestSimpleUnboundedNoEviction(t *testing.T) {
	numbers := 1000
	sizeTracker := 0
	gcu := buildTestLoadingCache(t, cacheTypeSimple, 0, loader)
...
func TestSimpleGetIFPresent(t *testing.T) {
	testGetIFPresent(t, cacheTypeSimple)
}

func TestSimpleHas(t *testing.T) {
	gc := buildTestLoadingCacheWithExpiration(t, cacheTypeSimple, 2, 10*time.Millisecond)
