...
func TestLFUGet(t *testing.T) {
	...
	gc := buildTestLoadingCache(t, cacheTypeLFU, size, loader)
...
func TestLFULength(t *testing.T) {
	gc := buildTestLoadingCache(t, cacheTypeLFU, 1000, loader)
	gc.Get(ctx, "test1")
	gc.Get(ctx, "test2")
	length := gc.Len(true)
	expectedLength := 2
...
func TestLFUEvictItem(t *testing.T) {
	cacheSize := 10
	numbers := 11
	gc := buildTestLoadingCache(t, cacheTypeLFU, cacheSize, loader)
...
func TestLFUGetIFPresent(t *testing.T) {
	testGetIFPresent(t, cacheTypeLFU)
}

func TestLFUHas(t *testing.T) {
	gc := buildTestLoadingCacheWithExpiration(t, cacheTypeLFU, 2, 10*time.Millisecond)
